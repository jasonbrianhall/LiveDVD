...
OPTIONS:
  ...
  -t <target>      Target path for output data
EOOPTS
    exit 1
}
...
mkdir $target/etc/yum.repos.d/ -p
...
then
    ...
    yum -c "$yum_config" --installroot="$target" --releasever=/ --setopt=tsflags=nodocs -y install $install_packages
fi
